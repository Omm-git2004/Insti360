...
const PunchIn = () => {
    ...
    const punchNow = () => {
        ...
        axios.post("punchId/punchIn", { departmentId: departmentData._id, RFIDTagId: rfidTag })
            .then(res => {
            ...
            .catch(err => {
                console.error(`Error at PunchIn --> ${err}`)
        ...
        departmentData
            ? <div className="scan-page" onClick={() => document.getElementById("rfidHiddenInput")?.focus()}>
                ...
                <input
                    ...
                    onKeyDown={(e) => {
                        if (e.key === "Enter") punchNow();  // ✅ punchIn or punchOut
                    }}
                    autoFocus
                ...
                {!punchedStudent ? (
                    <div className="scan-card">
                        ...
                        <p className="scan-click">Tap to Punch In</p>
                    </div>
                ) : (
                    <div className="scan-card success-card">
                        <h2 className="success-title">✅ Punch Successful</h2>
                        <h3 className="success-name">
                            Welcome, {punchedStudent?.studentName} 🎉
                        </h3>

                        <p className="success-text">ID: {punchedStudent?.studentId}</p>
                        <p className="success-text">
                            Entry Time: {punchedStudent?.entryTime}
                        </p>
